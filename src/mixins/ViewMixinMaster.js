...
export default {
  ...
  methods: {
    ...
    addRecord () {
      this.$router.push({ name: `new${this.pascalCaseSigular}`, params: { isAdd: true } })
    },
    editRecord (record, index) {
      let id = record.id
      this.$router.push({ name: `edit${this.pascalCaseSigular}`, params: { id } })
    },
    async deleteRecordConfirm (record) {
      ...
      this.$vm.dialog({
        ...
        text: this.$t('front.delete_confirmation', { attribute: this.$t(record.folio) }),
        accept: this.auxDeleteRecord
      })
    },
    async deleteRecordsConfirm () {
      let t = this
      let recordsMany = parseDeleteMany(this.selected, 'folio')
      this.auxDeleteRecord = () => {
      ...
      this.$vm.dialog({
        color: 'danger',
        title: this.$t('front.delete_modal'),
        text: this.$t('front.delete_confirmation', { attribute: recordsMany.values }),
        accept: this.auxDeleteRecord
      })
    },
    ...
    async getRecords (page) {
      await this.loadRecords({ page })
    },
    async getCatalog () {
      Object.keys(this.catalog).forEach(e => this.loadCatalog({ url: e, filter: '' }))
    },
